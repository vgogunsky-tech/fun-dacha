...
INSERT INTO oc_product_option_value (product_option_id, product_id, option_id, option_value_id, quantity, subtract, price, price_prefix, points, points_prefix, weight, weight_prefix) SELECT @poid, @pid, @opt_id, @ov_large, 100, 1, 20.00, '+', 0, '+', 0, '+' FROM DUAL WHERE @poid IS NOT NULL AND ((SELECT 1 FROM DUAL WHERE 1=1) OR 1=1);
UPDATE oc_product SET quantity = 300, stock_status_id = (CASE WHEN 300 > 0 THEN 5 ELSE 8 END) WHERE product_id=@pid;
...
INSERT INTO oc_product_option_value (product_option_id, product_id, option_id, option_value_id, quantity, subtract, price, price_prefix, points, points_prefix, weight, weight_prefix) SELECT @poid, @pid, @opt_id, @ov_large, 100, 1, 20.00, '+', 0, '+', 0, '+' FROM DUAL WHERE @poid IS NOT NULL AND ((SELECT 1 FROM DUAL WHERE 1=1) OR 1=1);
UPDATE oc_product SET quantity = 200, stock_status_id = (CASE WHEN 200 > 0 THEN 5 ELSE 8 END) WHERE product_id=@pid;
