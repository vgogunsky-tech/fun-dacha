#!/usr/bin/env python3
"""
Update Migration for Proper Language Support
This script updates the migration to use proper Ukrainian (2) and Russian (3) language IDs
"""
...
def update_migration_for_languages():
    """Update migration SQL to use proper language IDs"""
    
    print("🔧 Updating migration for proper language support...")
    ...
    sql_content = f"""-- Complete Sync OpenCart Migration SQL with Proper Language Support
-- Generated from actual CSV data with Ukrainian (2) and Russian (3) language support
...
"""
    ...
    sql_content += "\n-- Insert category descriptions (Ukrainian - language_id = 2)\n"
    for old_id, cat_data in categories.items():
    ...
    sql_content += "\n-- Insert category descriptions (Russian - language_id = 3)\n"
    for old_id, cat_data in categories.items():
        cat_id = category_mapping[old_id]
        name = cat_data['name'].replace("'", "\\'")
        description = cat_data['description'].replace("'", "\\'")
        ...
        sql_content += f"INSERT INTO oc_category_description (category_id, language_id, name, description, meta_title, meta_description, meta_keyword) VALUES ({cat_id}, 3, '{name}', '{description}', '{name}', '{meta_desc}', '{tag}');\n"
    ...
    # Add product descriptions with localized tags (Ukrainian)
    sql_content += "\n-- Insert product descriptions (Ukrainian - language_id = 2) with localized tags\n"
    product_id = 1
    for product in products:
    ...
    # Add product descriptions with localized tags (Russian)
    sql_content += "\n-- Insert product descriptions (Russian - language_id = 3) with localized tags\n"
    product_id = 1
    for product in products:
        name_ru = product.get('Название (рус)', '').replace("'", "\\'")
        desc_ru = product.get('Описание (рус)', '').replace("'", "\\'")
        tags_string = product.get('tags', '')
        tags_ru = localize_tags(tags_string, 3).replace("'", "\\'")
        
        meta_desc_ru = desc_ru[:250] if len(desc_ru) > 250 else desc_ru
        
        sql_content += f"INSERT INTO oc_product_description (product_id, language_id, name, description, tag, meta_title, meta_description, meta_keyword) VALUES ({product_id}, 3, '{name_ru}', '{desc_ru}', '{tags_ru}', '{name_ru}', '{meta_desc_ru}', '{tags_ru}');\n"
        product_id += 1
    ...
    if attributes:
        ...
        sql_content += "\n-- Insert attribute descriptions (Ukrainian - language_id = 2)\n"
        attribute_id = 1
        for attr in attributes:
        ...
        sql_content += "\n-- Insert attribute descriptions (Russian - language_id = 3)\n"
        attribute_id = 1
        for attr in attributes:
            name_ru = attr.get('ru', '').replace("'", "\\'")
            sql_content += f"INSERT INTO oc_attribute_description (attribute_id, language_id, name) VALUES ({attribute_id}, 3, '{name_ru}');\n"
            attribute_id += 1
    
    # Add display fixes and language settings
    sql_content += f"""
...
UPDATE oc_setting SET value = '2' WHERE `key` = 'config_language' AND store_id = 0;
...
(0, 'config', 'config_name_2', 'Фун-дача', 0),
(0, 'config', 'config_name_3', 'Фун-дача', 0),
(0, 'config', 'config_owner_2', 'Фун-дача', 0),
(0, 'config', 'config_owner_3', 'Фун-дача', 0),
(0, 'config', 'config_address_2', 'Україна', 0),
(0, 'config', 'config_address_3', 'Украина', 0),
(0, 'config', 'config_meta_title_2', 'Фун-дача - Сімена та садівництво', 0),
(0, 'config', 'config_meta_title_3', 'Фун-дача - Семена и садоводство', 0),
(0, 'config', 'config_meta_description_2', 'Інтернет-магазин сімен та садівничих товарів', 0),
(0, 'config', 'config_meta_description_3', 'Интернет-магазин семян и садоводческих товаров', 0);
"""
    
    # Write SQL file
    with open('complete_sync_migration_with_languages.sql', 'w', encoding='utf-8') as f:
        f.write(sql_content)
    
    print(f"✅ Generated complete_sync_migration_with_languages.sql with:")
    print(f"   - {len(categories)} categories with Ukrainian (2) and Russian (3) descriptions")
    print(f"   - {len(products)} products with Ukrainian (2) and Russian (3) descriptions")
    print(f"   - {len(attributes)} attributes with Ukrainian (2) and Russian (3) descriptions")
    print(f"   - Proper language setup (Ukrainian = 2, Russian = 3)")
    print(f"   - Localized tags (Ukrainian/Russian)")
    print(f"   - Language-specific store settings")
    print(f"   - Default language set to Ukrainian")
    
    return True

if __name__ == "__main__":
    update_migration_for_languages()